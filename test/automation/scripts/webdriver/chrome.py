...
class ChromeDriverManager(WebDriverManagerBase):
    ...
    def get_download_url(self, version="latest"):
        ...
        if not entry:
            raise Exception(f"Error, unable to find appropriate download for {self.os_name}{local_bitness}, version {version}.")
    ...
    def get_latest_version(self):
        ...
        if resp.status_code != 200:
            raise Exception(f"Error, unable to get version number for latest release, got code: {resp.status_code}")
    ...
    def _get_browser_version(self):
        ...
        raise RuntimeError("Unable to find Chrome on this system, please install it first!")